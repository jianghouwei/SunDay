...
function initUploadControl() {
	...
	var settings = {
		...
		upload_start_handler: function () {
			alert(uploadImgs.length);
			var imgSize = uploadImgs.length;
			if (imgSize == 5) {
		...
		upload_progress_handler: function () {
			 showInputTips('mscolluploadImg-info',  '图片上传中...', 'btnCancel', 'button'); 
		},
		...
		upload_success_handler: function (f, d, r) {
			...
			if (d != null) {
				...
				for (var i = 0; i < uploadImgs.length; i++) {
					var img = uploadImgs[i];
					//img = d ;
					var imgSrc ='http://localhost:8080/SunDay/upload/'+ img;
					var thumbnail ='http://localhost:8080/SunDay/upload/'+img;
